...
while( parar < 500000)
    X1 = X ;			%atualizo minha solução
    for i= 1:n			%enquanto i não chega no numero de equações	
    	X1(i,1) = D(i,:)*X1 + I(i,1);% Atualizo minha solução para todas as linhas
    end
    for i = 1:n 			%enquanto i não chega no numero de equações
        er(i,1) = abs((X1(i,1) -X(i,1))/X1(i,1))*100;%verifico o erro da minha solução
        merro(i,1) = erro; 	%crio um vetor de erros
    end
    if er < merro 		%se o erro for aceitável
        fprintf('%1.5f\n',X1);	%armazeno dado no formato correto
        break			%e saio do loop
    end
    X =X1;			%se não atualizo solução e recomeço 
    k = k+1;			%e somo no número de iterações
    %parar = parar+1;
end
disp('x seidel:');		%output a resposta pelo método seidel.m
 
disp(X');
 
disp('k seidel: ');		%output do número de iterações
